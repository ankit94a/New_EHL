...
export class TechManualsAddComponent {
  ...
  eqpt:number;
  constructor(private authService:AuthService, @Inject(MAT_DIALOG_DATA) data,private dialogRef:MatDialogRef<TechManualsAddComponent>, private apiService: ApiService,private fb: FormBuilder,private toastr: ToastrService,private dailogRef: MatDialogRef<TechManualsAddComponent>) {
  ...
  bindDataToForm(policyData){
    ...
    this.policy = this.fb.group({
      ...
      categoryId: [{value:policyData.categoryId,}, [Validators.required]],
      policyFile: [null, [Validators.required]],
      remarks: [{value:policyData.remarks,disabled:false},[Validators.required]],
    });
  }
  createForm() {
    this.policy = this.fb.group({
    ...
    this.apiService
      ...
      .subscribe((res) => {
        if (res) {
          ...
          if(this.apiUrl == 'policy/update'){
            this.policy.get('subCategoryId')?.setValue(this.categoryId);
            this.getEqpt(this.categoryId)
          }
...
//
    ...
    if (this.policy.valid) {
      // formData.append('id',this.policy.get('id')?.value.value)
      formData.append('id', this.policy.get('id')?.value ? this.policy.get('id')?.value.value : '0');
    ...
    this.apiService
      ...
      .subscribe((res) => {
        if (res) {
          ...
          if(this.apiUrl == 'policy/update'){
            this.policy.get('categoryId')?.setValue(this.categoryId);
