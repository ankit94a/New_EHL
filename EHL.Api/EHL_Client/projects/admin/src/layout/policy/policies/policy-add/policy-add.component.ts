...
export class PolicyAddComponent {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<PolicyAddComponent>,
  ...
  createForm() {
    this.policy = this.fb.group({
      type: [{ value: 'Policy Compendium', disabled: true }, [Validators.required]],
 ...
 async save() {
    ...
    if (policyId > 0) {
      ...
      if (isValid) {
        ...
        formData.append('type', 'Policy Compendium');
       ...
       const rawObject = {
        ...
        this.apiService.postWithHeader(this.apiUrl, formData).subscribe({
          next: (res) => {
            this.toastr.success('Policy Compendium submitted successfully', 'Success');
            this.dialogRef.close(true);
          },
          error: (err) => {
            this.toastr.error('Error submitting Policy Compendium', 'Error');
          },
        });
      } else {
