...
@Component({
  selector: 'app-add-eqpt-appreciation',
  standalone: true,
  imports: [SharedLibraryModule],
  templateUrl: './add-eqpt-appreciation.component.html',
  styleUrl: './add-eqpt-appreciation.component.scss'
})
export class AddEqptAppreciationComponent {
  policy: FormGroup;
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<AddEqptAppreciationComponent>,
  ...
  createForm() {
    this.policy = this.fb.group({
      type: [{ value: 'Eqpt Appreciation', disabled: true }, [Validators.required]],
  ...
  getEqpt(subCategoryId) {
    ...
    if (policyId > 0) {
      ...
      if (isValid) {
        ...
        formData.append('type', 'Eqpt Appreciation');
       ...
       const rawObject = {
        ...
        this.apiService.postWithHeader(this.apiUrl, formData).subscribe({
          next: (res) => {
            this.toastr.success('Eqpt Appreciation submitted successfully', 'Success');
